...
# we need to do this because we only have 9 years of data but need a longer-term climatology
# to calculate heatwaves from
...
# use the model to predict temperature based on the testing dataset
...
# check how well we model extremes
summary(lm(data = sonde.SP.woodruff.testing %>% filter(mean_temp >= quant.90), modeled~mean_temp))

# the model does not model extremes as well, but that is okay because we are using it 
# as a mean baseline
...
### make ASLO plot of modeled temp for each lake ####
...
#### plot comparing the predicted temperature to the actual temperature ####
...
# using a subset of years that have good data coverage from Sparkling
...
# format modeled temp data for heatwaveR
...
### calculate climatology and thresholds from modeled temperature data
...
### calculate climatology and thresholds again using categorical outputs of hw intensity
modeled.climOutputL = ts2clm(modeled.heatwaveR.L, climatologyPeriod = c(min(modeled.heatwaveR.L$t), max(modeled.heatwaveR.L$t)))
...
# filter to 1990 because the long-term climatology is the same across all years
...
##### Make a new version of the heatwaves dataset using the new modeled heatwaves #####
...
#================================================================================================================#
#================================================================================================================#
######## ALTERNATIVE MODELS THAT WERE NOT USED ############
#================================================================================================================#
