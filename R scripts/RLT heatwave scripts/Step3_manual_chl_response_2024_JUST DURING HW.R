#' this code is intended to work just like step3_manual_chl_response_2024.R,
#' But instead of calculating dates based on the end of the heatwave, it calculates
#' dates based on the beginning of the heatwave
#' 
...
daysAfterStart = 5 # time lag of how many days after the start of the heatwave we want to look
...
plot_metadata <- data.frame(slopeLength = paste("Length of each slope: ", slopeLength, sep = ""),
                            ...
                            daysAfterStart = paste("Number of days after start of heatwave: ", daysAfterStart, sep = ""),
...
metadata_plot <- ggplot() +
  ...
  geom_text(data = plot_metadata, aes(x = 5, y = 5, label = daysAfterStart),
...
for(slopeLength in 3:14){
  for(daysAfterStart in 0:20){
    for(numSlopes in 3:14){
      ...
      # comes from Step2_prep_chl_for_slopes_calc.R
      ...
      for(i in 1:length(lake_years)){
        ...
        # changed this line to be .after
        models <- slide(
          temp, 
          ~lm(mean_chl ~ doyCat, data = .x), 
          .after = slopeLength -1, # add the minus one so the slopeLength matches the user input
          .complete = TRUE
        )
      ...
      hwSlopes <- function(heatwaveStart, heatwaveEnd,  targLake, data){
        ...
        # CHANGED this to be the start date of the heatwave, instead of the end date of the heatwave
        startDate = as.Date(heatwaveStart)+daysAfterStart 
        endDate = as.Date(heatwaveStart)+daysAfterStart+numSlopes-1
      ...
      #   annotate("text",  x=Inf, y = Inf, label = paste("Days after heatwave: ", daysAfterStart, sep = ""), vjust=1, hjust=1)+
      ...
      # and includes the number of days after the heatwave event (daysAfterStartLoop) that is considered
      # one column, "exclude after heatwaves," are slopes within 40 days after a heatwave
      # that are not in the current rolling window
      
      for(daysAfterStartLoop in 0:20){
        ...
        for(i in 1:nrow(heatwaves)){
          ...
          # after heatwave because of our current daysAfterStartLoop selection.
          ...
          # daysAfterStartLoop is set to, to that same value plus the numSlopes. So it sets a window daysAfterStartLooped
          # X number of days after the heatwave
          
          datesAnalyzed = seq(end+daysAfterStartLoop, end + daysAfterStartLoop+numSlopes-1, 1)
          ...
          slopes$daysAfterStart = daysAfterStartLoop
        }
        
        # combine to the main dataframe
        if(daysAfterStartLoop == 0){
          allSlopes = slopes
        }
        if(daysAfterStartLoop > 0){
      ...
      # calculate mean values by period and daysAfterStart
      ...
      mean_df <- allSlopes %>% 
        ...
        group_by(period, daysAfterStart) %>% 
      ...
      # being careful to specify daysAfterStart as the global environment variable
      # and not the column
      
      looped.results$all.all.other.days[L] = mean_df %>%
        filter(daysAfterStart == get("daysAfterStart", envir=globalenv()) & period == "all other days") %>%
        pull(mean_percent_change)
      
      # looped.results$all.during.heatwave[L] = mean_df %>%
      #           filter(daysAfterStart == get("daysAfterStart", envir=globalenv()) & period == "during heatwave") %>%
      #           pull(mean_percent_change)
      
      looped.results$all.after.heatwave[L] = mean_df %>%
        filter(daysAfterStart == get("daysAfterStart", envir=globalenv()) & period == "after heatwave") %>%
        pull(mean_percent_change)
...
All.over.time <- mean_df %>% filter(daysAfterStart > 0) %>% 
  ggplot(aes( x= daysAfterStart, y = mean_percent_change, color = period))+
  ...
  labs(x = "days after heatwave start")+
  theme_classic()

R.over.time <- mean_dfR %>% filter(daysAfterStart > 0) %>% 
  ggplot(aes( x= daysAfterStart, y = mean_percent_change, color = period))+
  geom_line(size = 1)+
  geom_point()+
  labs(x = "days after heatwave start")+
...
L.over.time <- mean_dfL %>% filter(daysAfterStart > 0) %>% 
  ggplot(aes( x= daysAfterStart, y = mean_percent_change, color = period))+
  geom_line(size = 1)+
  geom_point()+
  labs(x = "days after heatwave start")+
...
T.over.time <- mean_dfT %>% filter(daysAfterStart > 0) %>% 
  ggplot(aes( x= daysAfterStart, y = mean_percent_change, color = period))+
  scale_color_manual(values = c("during heatwave" = "#ff0000", "after heatwave" = "#ffc100", "all other days" = "#88CCEE"))+
  #scale_fill_manual(values = c("during heatwave" = "#ff0000", "after heatwave" = "#ffc100", "all other days" = "#88CCEE"))+
  geom_line(size = 1)+
...
Alldist <- allSlopes %>%
  filter(period != "exclude after heatwave", daysAfterStart == get("daysAfterStart", envir=globalenv())) %>%
  ...
  geom_density_ridges(alpha = 0.7,
                      ...
                      scale = 2, size = 0.7) +
  geom_text(data = mean_df %>% filter(daysAfterStart == get("daysAfterStart", envir=globalenv())),
...
# 
# allSlopes %>%
#   filter(period != "exclude after heatwave", lake == "L", daysAfterStart == get("daysAfterStart", envir=globalenv())) %>%
...
#   theme_classic() +
#   geom_text(data = mean_dfL %>% filter(daysAfterStart == get("daysAfterStart", envir=globalenv())),
#             aes(x = mean_percent_change,
...
Tdist <- allSlopes %>%
  ...
  geom_density_ridges(alpha = 0.7,
                      ...
                      quantile_fun = function(x, ...) mean(x), 
                      scale = 2, size = 0.7) +
  geom_text(data = mean_dfT %>% filter(daysAfterStart == get("daysAfterStart", envir=globalenv())),
            aes(x = mean_percent_change,
                y = factor(period, levels = desired_order),  # Use factor with desired order
                label = as.character(round(mean_percent_change, digits = 0))),
            color = "black",
...
Rdist <- allSlopes %>%
  ...
  geom_density_ridges(alpha = 0.7,
                      ...
                      quantile_fun = function(x, ...) mean(x), 
                      scale = 2, size = 0.7) +
  geom_text(data = mean_dfR %>% filter(daysAfterStart == get("daysAfterStart", envir=globalenv())),
            aes(x = mean_percent_change,
                y = factor(period, levels = desired_order),  # Use factor with desired order
                label = as.character(round(mean_percent_change, digits = 0))),
            color = "black",
...
pdfName = paste(runNumber, "_date_", curDate, "_slopeLength_", slopeLength, "_daysAfterStart_", daysAfterStart, "_numSlopes_", numSlopes, ".pdf", sep = "")
...
makePDFrawPlots(allSlopes, daysAfterStart, numSlopes, metadata_plot, runNumber, slopeLength)
...
# pass it allSlopes, daysAfterStart, numSlopes, slopeLength, runNumber, along with the output information
...
# 
# Rdist<- allSlopes %>%
#   filter(period != "exclude after heatwave", lake == "R", daysAfterStart == get("daysAfterStart", envir=globalenv())) %>%
#   ggplot(aes(x = percent_change, y = factor(period, levels = desired_order), fill = stat(x))) +
...
# Tdist <- 
#   allSlopes %>%
#   filter(period != "exclude after heatwave", lake == "T", daysAfterStart == get("daysAfterStart", envir=globalenv())) %>%
#   ggplot(aes(x = percent_change, y = factor(period, levels = desired_order), fill = stat(x))) +
#   geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01, quantile_lines = TRUE,
#                                quantile_fun = function(x, ...) mean(x), 
#                                size = 0.7, alpha = 0.7) +
...
# Peter
R.after.days.num <- ggplot(data = looped.results, aes(x = daysAfterStart, y = numSlopes, fill = R.after.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

R.after.days.slope <- ggplot(data = looped.results, aes(x = daysAfterStart, y = slopeLength, fill = R.after.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

R.after.num.slope <- ggplot(data = looped.results %>% filter(daysAfterStart == 7), aes(x = slopeLength, y = numSlopes, fill = R.after.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

# Paul
L.after.days.num <- ggplot(data = looped.results, aes(x = daysAfterStart, y = numSlopes, fill = L.after.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

L.after.days.slope <- ggplot(data = looped.results, aes(x = daysAfterStart, y = slopeLength, fill = L.after.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

L.after.num.slope <- ggplot(data = looped.results %>% filter(daysAfterStart == 7), aes(x = slopeLength, y = numSlopes, fill = L.after.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

# Tuesday
T.after.days.num <- ggplot(data = looped.results, aes(x = daysAfterStart, y = numSlopes, fill = T.after.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

T.after.days.slope <- ggplot(data = looped.results, aes(x = daysAfterStart, y = slopeLength, fill = T.after.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

T.after.num.slope <- ggplot(data = looped.results %>% filter(daysAfterStart == 7), aes(x = slopeLength, y = numSlopes, fill = T.after.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")
...
# Peter
R.during.days.num <- ggplot(data = looped.results, aes(x = daysAfterStart, y = numSlopes, fill = R.during.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

R.during.days.slope <- ggplot(data = looped.results, aes(x = daysAfterStart, y = slopeLength, fill = R.during.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")
...
# Paul
L.during.days.num <- ggplot(data = looped.results, aes(x = daysAfterStart, y = numSlopes, fill = L.during.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

L.during.days.slope <- ggplot(data = looped.results, aes(x = daysAfterStart, y = slopeLength, fill = L.during.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")
...
# Tuesday
T.during.days.num <- ggplot(data = looped.results, aes(x = daysAfterStart, y = numSlopes, fill = T.during.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

T.during.days.slope <- ggplot(data = looped.results, aes(x = daysAfterStart, y = slopeLength, fill = T.during.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

T.during.num.slope <- ggplot(data = looped.results %>% filter(daysAfterStart == 7), aes(x = slopeLength, y = numSlopes, fill = T.during.heatwave))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")
...
# Peter
R.other.days.num <- ggplot(data = looped.results, aes(x = daysAfterStart, y = numSlopes, fill = R.all.other.days))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

R.other.days.slope <- ggplot(data = looped.results, aes(x = daysAfterStart, y = slopeLength, fill = R.all.other.days))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")
...
# Paul
L.other.days.num <- ggplot(data = looped.results, aes(x = daysAfterStart, y = numSlopes, fill = L.all.other.days))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

L.other.days.slope <- ggplot(data = looped.results, aes(x = daysAfterStart, y = slopeLength, fill = L.all.other.days))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")
...
# Tuesday
T.other.days.num <- ggplot(data = looped.results, aes(x = daysAfterStart, y = numSlopes, fill = T.all.other.days))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

T.other.days.slope <- ggplot(data = looped.results, aes(x = daysAfterStart, y = slopeLength, fill = T.all.other.days))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")

T.other.num.slope <- ggplot(data = looped.results %>% filter(daysAfterStart == 7), aes(x = slopeLength, y = numSlopes, fill = T.all.other.days))+
  geom_tile(color = "black")+
  scale_fill_gradientn(colors = hcl.colors(20, "Spectral"), trans = "reverse")
