...
event_line_doy <- function(data,
  ...
  ep <- ggplot(data = clim_spread, aes(x = as.numeric(format(ts_x, "%j")), y = ts_y)) +
  ...
  if (category) {
    ...
    if (event_top$intensity_mean < 0) {
      ...
      ep <- ep +
        geom_flame(data = clim_events, linewidth = 0.5,
                   aes(x = as.numeric(format(ts_x, "%j")), y = thresh, y2 = ts_y, fill = "Moderate")) +
        geom_flame(data = clim_events, linewidth = 0.5,
                   aes(x = as.numeric(format(ts_x, "%j")), y = thresh_2x, y2 = ts_y, fill = "Strong")) +
        geom_flame(data = clim_events, linewidth = 0.5,
                   aes(x = as.numeric(format(ts_x, "%j")), y = thresh_3x, y2 = ts_y, fill = "Severe")) +
        geom_flame(data = clim_events, linewidth = 0.5,
                   aes(x = as.numeric(format(ts_x, "%j")), y = thresh_4x, y2 = ts_y, fill = "Extreme"))
    } else {
      fillColCat <- c(
        "Moderate" = "#ffc866",
        "Strong" = "#ff6900",
      ...
      ep <- ep +
        geom_flame(data = clim_events, linewidth = 0.5,
                   aes(x = as.numeric(format(ts_x, "%j")), y = y1, y2 = y2, fill = "Moderate")) +
        geom_flame(data = clim_events, linewidth = 0.5,
                   aes(x = as.numeric(format(ts_x, "%j")), y = y1, y2 = thresh_2x, fill = "Strong")) +
        geom_flame(data = clim_events, linewidth = 0.5,
                   aes(x = as.numeric(format(ts_x, "%j")), y = y1, y2 = thresh_3x, fill = "Severe")) +
        geom_flame(data = clim_events, linewidth = 0.5,
                   aes(x = as.numeric(format(ts_x, "%j")), y = y1, y2 = thresh_4x, fill = "Extreme"))
    }
    ...
    ep <- ep +
      geom_flame(data = clim_events, linewidth = 0.5,
                 aes(x = as.numeric(format(ts_x, "%j")), y = y1, y2 = y2, fill = "events")) +
      geom_flame(data = clim_top, linewidth = 0.5,
                 aes(x = as.numeric(format(ts_x, "%j")), y = y1, y2 = y2, fill = "peak event")) +
      geom_line(aes(y = seas, col = "Climatology"),
                linewidth = 0.7, alpha = 1) +
      geom_line(aes(y = thresh, col = "Threshold"),
                linewidth = 0.7, alpha = 1) +
