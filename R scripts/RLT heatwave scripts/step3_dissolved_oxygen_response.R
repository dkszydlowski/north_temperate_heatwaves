...
for(i in 1:length(lake_years)){
  ...
  for(j in 1:nrow(temp)){
    if(is.na(temp$mean_doSat[j+indexChange])){
...
daysBefore = 7
  ...
  for(i in 1:length(lake_years)){
    ...
    for(j in 1:nrow(temp)){
      ...
      if(!(is.null(coef))){
        Slope <- coef["doyCat"]
        temp$doSat_slope[j] = Slope # pull out the slope from the model
...
hwSlopes <- function(heatwaveStart, heatwaveEnd,  targLake, data){
  ...
  # change in doSatorophyll
  
  beforeDate = as.Date(heatwaveStart) -1
  
  # get the starting doSatorophyll concentration before the heatwave
  
  startdoSat = data %>% filter(date == beforeDate) %>% 
    select(mean_doSat)
  ...
  # put the starting doSatorophyll value in the dataframe so we can return it with the function
  
  temp = temp %>% mutate(doSat_before = startdoSat$mean_doSat[1])
  return(temp)
...
for(i in 1:lengthHW){
  ...
  heatwaves$averageSlope[i] = mean(test$doSat_slope, na.rm = TRUE)
  heatwaves$percentChange[i] = 100*(mean(test$doSat_slope, na.rm = TRUE)*7)/test$doSat_before[1]
...
allSlopes %>%
  ...
  geom_text(data = mean_dfT %>% filter(shift == 1),
            ...
            vjust = 2) +
  ylab("") +
  xlab("% change in surface dissolved oxygen saturation")+
  labs(title = "Tuesday") +
...
allSlopes %>%
  filter(lake == "L") %>%
  filter(period != "exclude after heatwave", shift == 7) %>%
  ggplot(aes(x = percent_change,
             y = factor(period, levels = desired_order),  # Use factor with desired order
             fill = period)) +
  geom_density_ridges(alpha = 0.7,
                      ...
                      quantile_fun = function(x, ...) mean(x), 
                      scale = 2, size = 0.7) +
  geom_text(data = mean_dfR %>% filter(shift == 7),
            aes(x = mean_percent_change,
                y = factor(period, levels = desired_order),  # Use factor with desired order
                label = as.character(round(mean_percent_change, digits = 0))),
            color = "black",
